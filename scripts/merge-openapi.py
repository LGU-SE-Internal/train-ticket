...
def merge_openapi_docs(
    docs: List[Dict[str, Any]], output_title: str = "Train Ticket API"
) -> Dict[str, Any]:
    """
    ...
    merged = {
        ...
        "info": {
            ...
            "version": "1.0.0",
            "contact": {"name": "Train Ticket Team"},
        },
        "servers": [
            {
    ...
    for service_name, doc in docs:
        ...
        service_prefix = (
            service_name.replace("ts-", "").replace("-service", "").replace("-", "_")
        )
        ...
        if "tags" in doc:
            for tag in doc["tags"]:
                ...
                if tag_name and tag_name not in seen_tags:
                    ...
                    # 添加服务来源信息
                    tag_copy = tag.copy()
                    tag_copy["description"] = (
                        f"[{service_name}] {tag.get('description', '')}"
                    )
            ...
            for path, methods in doc["paths"].items():
                ...
                for method, operation in methods.items():
                    if method in merged["paths"][path]:
                        # 路径冲突，添加服务前缀
                        print(
                            f"警告: 路径冲突 {method.upper()} {path}，来自 {service_name}"
                        )
                    ...
                    if "operationId" in op_copy:
                        op_copy["operationId"] = (
                            f"{service_prefix}_{op_copy['operationId']}"
                        )
        ...
        if "components" in doc and "schemas" in doc["components"]:
            for schema_name, schema_def in doc["components"]["schemas"].items():
                if schema_name in seen_schemas:
                    # 检查是否相同
                    if json.dumps(
                        seen_schemas[schema_name], sort_keys=True
